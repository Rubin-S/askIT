...
function NavBar() {
  ...
  const [password, setPassword] = useState("");
   ...
   const togglePasswordVisibility = (e) => {
     if (e) e.preventDefault(); // Stop propagation
...
const handleEmailChange = (e) => {
  ...
  const closeLogin = () => {
    setShowLogin(false);
    navigate("/", { replace: true }); // removes #login
  };
  ...
  const handle_manual_login = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        handleClosePopup(); // Close the login popup
      }
    } catch (error) {
      if (error.response) {
        toast.error(
        ...
        <div className="login-card" ref={loginRef}>
          {!showEmailLogin ? (
            <>
              ...
              <button
                className="login-stay-logged-out-btn"
